...
const GuestDashboard: React.FC<GuestDashboardProps> = ({ property, access, onCheckIn, onLogout, language, onToggleLanguage }) => {
  ...
  const DarkSection = ({ title, subtitle, items, icon }: { title: string, subtitle: string, items: string[], icon: React.ReactNode }) => (
    <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="relative z-10">
        ...
        <div className="mt-8 pt-6 border-t border-white/10 text-center"><p className="text-[9px] font-black tracking-[0.2em] text-white/30">HostFlow</p></div>
      </div>
    </div>
  );
  ...
  return (
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-24 font-sans overflow-x-hidden selection:bg-[#0052FF] selection:text-white">
      ...
      {/* ✅ CORREGIDO: <main> ahora está DENTRO del div raíz */}
            ...
            {property.aiRecommendations && Object.keys(property.aiRecommendations).length > 0 ? (
              <>
                <div className="relative mb-6 group">
                  ...
                  <div ref={scrollRef} onScroll={handleScroll} className="flex gap-3 overflow-x-auto pb-4 no-scrollbar scroll-smooth relative z-10">
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        <div className="flex justify-between items-start mb-6">
                          <span className="bg-white/5 backdrop-blur-md border border-white/10 text-white/50 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest">{place.type || activeCategory}</span>
                        ...
                        <h4 className="font-black text-white text-2xl leading-tight mb-3 tracking-tighter group-hover:text-[#0052FF] transition-colors">{place.name}</h4>
                        <p className="text-white/40 text-xs font-medium leading-relaxed mb-8 line-clamp-3">{place.description}</p>
                      </div>
              ...
              <div className="bg-white rounded-[3rem] p-16 text-center border-2 border-dashed border-slate-100 animate-pulse">
                ...
                <h4 className="text-xl font-black text-slate-800 mb-2">Preparando tu guía...</h4>
      ...
      {showDoorModal && (
        <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-slate-900/80 backdrop-blur-lg animate-in fade-in duration-300">
          <div className="bg-white w-full sm:max-w-md rounded-t-[3rem] sm:rounded-[3rem] p-8 pb-12 text-center shadow-2xl relative animate-in slide-in-from-bottom-20 duration-500">
            ...
            <div className="bg-slate-50 border-4 border-slate-100 rounded-[2.5rem] py-12 mb-8 relative overflow-hidden">
            ...
            <div className="flex flex-col items-center space-y-2 mb-8">
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{t.guest.modals.availableFor}</p>
              <p className="text-3xl font-mono font-black text-[#0052FF] tracking-tight">{formatTime(timeLeft)}</p>
            </div>
            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-center mb-10">
                <div className="p-4 bg-white rounded-3xl">
                  <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=WIFI:S:${property.wifiSSID};T:WPA;P:${property.wifiPass};;`} className="w-40 h-40 mix-blend-multiply" />
                </div>
              </div>
