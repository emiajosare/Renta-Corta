...
export default function HostDashboard({ user, onLogout, onStartCreating }: any) {
  const [showVideoModal, setShowVideoModal] = useState(false); // 🟢 Control del modal de video
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-73px)] overflow-hidden">
        ...
        <div className="w-full lg:w-1/2 p-8 lg:p-16 overflow-y-auto flex items-center">
          <div className="max-w-md mx-auto w-full">
            ...
            {/* Cabecera del formulario con Botón de Ayuda Integrado */}
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-4xl font-black italic">Estándar de Oro.</h2>
              
              {/* 🟢 El Botón de soporte: Visible, elegante y con llamado de acción suave */}
              <button 
                onClick={() => setShowVideoModal(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#C9A84C]/10 border border-[#C9A84C]/30 rounded-full text-[#C9A84C] text-[10px] font-black uppercase tracking-widest hover:bg-[#C9A84C]/20 transition-all animate-pulse"
              >
                <span>▶ Guía de Onboarding</span>
              </button>
            </div>
            ...
            {/* 🟢 VENTANA MODAL FLOTANTE DE VIDEO ASISTENTE */}
            {showVideoModal && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-in fade-in duration-300">
                <div className="bg-[#111] border border-white/10 p-6 rounded-[2.5rem] max-w-2xl w-full relative shadow-[0_0_50px_rgba(201,168,76,0.15)] animate-in zoom-in-95 duration-300">
                  ...
                  {/* Contenedor del reproductor HTML5 nativo para tu video en /public */}
                  <div className="relative aspect-video rounded-2xl overflow-hidden border border-white/10 bg-black shadow-inner">
                    <video 
                      className="w-full h-full object-cover"
                      src="/tutorial.mp4" // 🎥 Apunta directo a tu archivo en public/
                      controls
                      autoPlay
                    />
                  </div>
